...
export default function MostrarMedicamentos() {
    ...
    function toComponentB(medicamen,quandoTomar, variavel) {
        navigate('/usermedicarespec', { state: { medicamen: medicamen, altura_Tomar: quandoTomar,id_medicamento:variavel  } });
    }
    ...
    if (datas.length > 0) {
        ...
        listItems = datas.map((data, i) =>
            ...
            <Col className="contact" key={i}>
                ...
                <Button onClick={() => toComponentB(data.medicamento,data.momento_tomar,variavel)} >   Editar  </Button>
            </Col>
            ...
            <Row>
                <Col xs={8}>
                    ...
                    <p className="blue paragraphInfo">
                        Pode verificar cada medicamento adicionado ao utilizador: <span className='darkgreen'> {email}</span>
                    </p>
                </Col>
